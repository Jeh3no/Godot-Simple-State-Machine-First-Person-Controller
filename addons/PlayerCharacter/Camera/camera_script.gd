...
@export_range(0.0, 0.5, 0.001) var x_axis_sensibility : float
@export_range(0.0, 0.5, 0.001) var y_axis_sensibility : float
@export_range(-360.0, 0.0, 0.01) var max_up_angle_view : float #in degrees
@export_range(0.0, 360.0, 0.01) var max_down_angle_view : float #in degrees
@export_range(5.0, 175.0, 0.01) var fov : float

@export_group("fov variables")
@export var min_fov_val : float
@export var max_fov_val : float
...
@export_range(-180.0, 180.0, 1.0) var zoom_val : float
@export_range(0.0, 3.0, 0.01) var zoom_duration : float
...
@export_range(0.0, 300.0, 0.1) var forward_move_tilt_divider : float #divider to add to the move speed calculated tilt value
@export_range(0.0, 7.0, 0.01) var forward_move_tilt_duration : float
@export_range(0.0, 2.0, 0.001) var forward_move_max_tilt_val : float
@export_range(0.0, 6.0, 0.1) var side_move_tilt_divider : float
@export_range(0.0, 24.0, 0.01) var side_move_tilt_speed : float
@export_range(0.0, 12.0, 0.001) var side_move_max_tilt_val : float
...
@export_range(0.0, 0.15, 0.001) var bob_pitch : float #in degrees
@export_range(0.0, 0.15, 0.001) var bob_roll : float #in degrees
@export_range(0.0, 1000.0, 1.0) var bob_height_divider : float
@export_range(2.0, 10.0, 0.1) var bob_frequency : float
@export_range(0.0, 1.0, 0.001) var cam_max_v_offset : float
@export_range(0.0, 15.0, 0.1) var cam_v_offset_to_0_speed : float
...
func _process(delta : float) -> void:
	state = play_char.stateMachine.currStateName
...
func lean(delta : float) -> void:
	...
	if enable_lean:
		if state in cam_lean_per_state.keys():
			camera.rotation.z = lerp(camera.rotation.z, deg_to_rad(cam_lean_per_state[state][0]), cam_lean_per_state[state][1] * delta)
		else:
			##keep this section of the code available if you want tilt as well
			#default camera rotation if no specific lean needs to be applied
			camera.rotation.z = lerp(camera.rotation.z, deg_to_rad(cam_lean_per_state["Default"][0]), cam_lean_per_state["Default"][1] * delta)
...
func tilt(delta : float) -> void:
	if state != "Fly" and state != "Slide":
		if enable_forward_tilt:
			...
			var has_started_moving_forward = sign(play_char.inputDirection.y) == 1 and sign(last_input_y) != 1
			var has_started_moving_backward = sign(play_char.inputDirection.y) == -1 and sign(last_input_y) != -1
			...
			if has_started_moving_forward or has_started_moving_backward:
				...
				var tilt_offset : float = clamp((-play_char.inputDirection.y * play_char.moveSpeed) / forward_move_tilt_divider, -forward_move_max_tilt_val, forward_move_max_tilt_val)
			...
			last_input_y = play_char.inputDirection.y
		...
		if enable_side_tilt:
			...
			clamp((-play_char.inputDirection.x * play_char.moveSpeed) / side_move_tilt_divider, -side_move_max_tilt_val, side_move_max_tilt_val), 
...
func bob(delta : float) -> void:
	...
	if enable_headbob and state != "Idle" and state != "Jump" and state != "Slide" and state != "Dash" and state != "Fly" and !play_char.ceilingCheck.is_colliding():
	...
	elif enable_headbob and (state == "Idle" or state == "Jump" or state == "Slide" or state == "Dash" or state == "Fly" or play_char.ceilingCheck.is_colliding()):
...
func change_fov() -> void:
	...
	prints(zoom_on, zoom_has_occured)
	...
	if !zoom_on and !zoom_has_occured:
		if state != null and state != "Jump" and state != "Inair":
			...
			if state != "Jump" and state != "Inair":
				...
				#that if you dezoom while being in Jump on Inair state, since these two states doesn't have a fixed fov
				#the fov would go back to the default one, even if play char jump with the Run state fov
				
				var walkOrRunState : String
				if play_char.walkOrRun == "WalkState":
					walkOrRunState = "Walk"
				elif play_char.walkOrRun == "RunState":
					walkOrRunState = "Run"
					
				fov_change_tween.tween_property(camera, "fov", cam_fov_per_state[walkOrRunState][0], cam_fov_per_state[walkOrRunState][1])
				fov_change_tween.finished.connect(Callable(fov_change_tween, "kill"))
